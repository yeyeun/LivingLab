...
public interface TeamRepository extends JpaRepository<Team, Integer> {
    ...
    // 검색 + 최신순
    @Query("select t, ti from Team t left join t.imageList ti where ti.ord = 0 and t.flag = false and t.title like %:title% order by t.createdDate desc")
    Page<Object[]> searchNewList(String title, Pageable pageable);

    // 검색 + 마감임박순
    @Query("select t, ti from Team t left join t.imageList ti where ti.ord = 0 and t.flag = false and t.title like %:title% order by t.deadline asc")
